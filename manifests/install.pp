...
class cygwin::install(
  ...
  # NOTE: In order to uninstall/install any packages we need a copy of
  # the 'setup' binary, so we'll copy this to Cygwin so we have
  # something we can rely on.
  file { "${install_root}\\bin\\setup.exe":
    ensure             => 'file',
    source             => "${::staging_windir}\\cygwin\\${installer}",
    source_permissions => 'ignore',
    mode               => '0755',
    require            => File["${::staging_windir}\\cygwin\\${installer}"],
  }
}
